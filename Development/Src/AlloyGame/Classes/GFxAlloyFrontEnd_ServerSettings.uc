...
Filename    :   GFxAlloyFrontEnd_ServerSettings.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_ServerSettings extends GFxAlloyFrontEnd_SettingsBase
    config(UI);
...
function SaveState()
{
	...
	local AlloyUIDataStore_StringList StringListDataStore;
        ...
        switch(ControlType)
        {
            case("stepper"):                                
				...
				if (SettingName == "ServerType")
				{
					...
					// LAN = 0 Internet = 1
					StringListDataStore.SetCurrentValueIndex(MatchTypeName, (StepperSelectedIndex == 0) ? class'GFxAlloyFrontEnd_HostGame'.const.SERVERTYPE_LAN : class'GFxAlloyFrontEnd_HostGame'.const.SERVERTYPE_UNRANKED);
				}
				else
				{
...
function UpdateListDataProvider()
{
    ...
    {        
		...
		if (!bFoundDefault)
		{
			if (LocationPart == "AlloyGameSettings")
			{
...
{   
    local int i, j;    
    local AlloyUIDataProvider_MenuOption CurrentSetting;
	local AlloyGameSettingsCommon GameSettings;

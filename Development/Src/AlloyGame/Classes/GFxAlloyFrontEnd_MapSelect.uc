...
Filename    :   GFxAlloyFrontEnd_MapSelect.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_MapSelect extends GFxAlloyFrontEnd_Screen
    config(UI)
    dependson(AlloyGame);
...
var array<AlloyUIDataProvider_MapInfo> MapList;
 ...
 */
function OnMapList_ValueChanged(String InMapSelected, optional string InMapImageSelected = class'GFxAlloyFrontEnd_LaunchGame'.const.MarkupForNoMapImage)
{
...
function UpdateListDataProvider()
{
	...
	local array<AlloyUIDataProvider_MapInfo> LocalMapList;
        
	// fill the local map list    
	MapList.Length = 0;
    LocalMapList.Length = 0;
	class'AlloyUIDataStore_MenuItems'.static.GetAllResourceDataProviders(class'AlloyUIDataProvider_MapInfo', ProviderList);
	for (i = 0; i < ProviderList.length; i++)
	{		
		LocalMapList.AddItem(AlloyUIDataProvider_MapInfo(ProviderList[i]));
	}   
    ...
    for (i = 0; i < LocalMapList.Length; i++)
    {
        if (!LocalMapList[i].ShouldBeFiltered())
        {
            ...
            else
            {
                ...
                TempObj.SetString("image", "img://" $ class'GFxAlloyFrontEnd_LaunchGame'.const.MarkupForNoMapImage);
            }
...
final static function String GetImageMarkupByMapName(String InMapName)
{
	...
	bResult = class'GFxAlloyFrontEnd_LaunchGame'.const.MarkupForNoMapImage;
...
final function SetupMapCycle(string SelectedMap)
{
	local int CycleIdx;
	local name GameMode;
	local AlloyGame.GameMapCycle MapCycle;
	...
	CycleIdx = class'AlloyGame'.default.GameSpecificMapCycles.Find('GameClassName', GameMode);
	if (CycleIdx == INDEX_NONE)
	{
		CycleIdx = class'AlloyGame'.default.GameSpecificMapCycles.length;
	}
	
	class'AlloyGame'.default.GameSpecificMapCycles[CycleIdx] = MapCycle;

	// Save the config for this class.
	class'AlloyGame'.static.StaticSaveConfig();
}
...
{    
    ...
    //`log("GFxAlloyFrontEnd_MapSelect: WidgetInitialized():: WidgetName: " @ WidgetName @ " : " @ WidgetPath @ " : " @ Widget);
    switch(WidgetName)
    {
        case ('list'):  
            if (ListMC == none)

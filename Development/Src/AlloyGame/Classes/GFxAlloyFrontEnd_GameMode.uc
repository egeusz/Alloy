...
Filename    :   GFxAlloyFrontEnd_GameMode.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_GameMode extends GFxAlloyFrontEnd_Screen
    config(UI);
...
var array<AlloyUIDataProvider_GameModeInfo> GameModeList;
...
function OnViewLoaded()
{
    ...
    SettingsDataStore = UIDataStore_OnlineGameSettings(DSClient.FindDataStore('AlloyGameSettings'));
	StringListDataStore = AlloyUIDataStore_StringList(DSClient.FindDataStore('AlloyStringList'));
        
	// fill the local game mode list
	class'UTUIDataStore_MenuItems'.static.GetAllResourceDataProviders(class'AlloyUIDataProvider_GameModeInfo', ProviderList);
	for (i = 0; i < ProviderList.Length; i++)
	{
	    GameModeList.InsertItem(0, AlloyUIDataProvider_GameModeInfo(ProviderList[i]));        
	}
}
...
final function OnGameModeSelected(string InGameMode, string InDefaultMap, optional string InGameSettingsClass = "")
{
	...
	MapImageMarkup = class'GFxAlloyFrontEnd_MapSelect'.static.GetImageMarkupByMapName(InDefaultMap);
	...
	class'GFxAlloyFrontEnd_Mutators'.static.ApplyGameModeFilter();    	
}
...
function UpdateListDataProvider()
{
    ...
    for (i = 0; i < GameModeList.Length; i++)
    {
        ...
        FriendlyGameModeName  = class'GFxAlloyFrontEnd_LaunchGame'.static.GetGameModeFriendlyString(GameModeList[i].GameMode);
...
{
    //`log("GFxAlloyFrontEnd_GameMode: WidgetInitialized():: WidgetName: " @ WidgetName @ " : " @ WidgetPath @ " : " @ Widget);

...
Filename    :   GFxAlloyFrontEnd_FilterDialog.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_FilterDialog extends GFxAlloyFrontEnd_Dialog
    config(UI);
...
var AlloyDataStore_GameSearch SearchDataStore;
...
/** Delegate for when the game type has been changed. This is set by the parent view, GFxAlloyFrontEnd_JoinGame. */
delegate transient OnSwitchedGameType();

function OnViewLoaded()
{
	// Retrieve references to relevant data stores.
	StringListDataStore = AlloyUIDataStore_StringList(class'UIRoot'.static.StaticResolveDataStore('AlloyStringList'));
	SearchDataStore = AlloyDataStore_GameSearch(class'UIRoot'.static.StaticResolveDataStore('AlloyGameSearch'));
}
...
function ValidateServerType()
{
	...
	else
	{
		...
		SetFilterNetworkType(ValueIndex == Class'GFxAlloyFrontEnd_JoinGame'.const.SERVERBROWSER_SERVERTYPE_LAN);
	}
}
...
function SetFilterNetworkType(bool bLAN, optional bool bSkipUIUpdate)
{
	...
	MatchTypeName = Class'WorldInfo'.Static.IsConsoleBuild(CONSOLE_XBox360) ? 'MatchType360' : 'MatchType';
	StringListDataStore.SetCurrentValueIndex(MatchTypeName, bLAN ? Class'GFxAlloyFrontEnd_JoinGame'.const.SERVERBROWSER_SERVERTYPE_LAN :
									Class'GFxAlloyFrontEnd_JoinGame'.const.SERVERBROWSER_SERVERTYPE_UNRANKED);

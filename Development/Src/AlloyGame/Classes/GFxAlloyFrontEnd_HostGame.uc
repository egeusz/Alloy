...
Filename    :   GFxAlloyFrontEnd_HostGame.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_HostGame extends GFxAlloyFrontEnd_LaunchGame
    config(UI);
...
function ValidateServerType()
{
	...
	if (!IsLoggedIn(PlayerControllerId, true) || class'WorldInfo'.static.IsConsoleBuild(CONSOLE_PS3))
	{
		...
		if ( ValueIndex != class'GFxAlloyFrontEnd_JoinGame'.const.SERVERBROWSER_SERVERTYPE_LAN )
		{
			// make sure the "LAN" option is selected
			StringListDataStore.SetCurrentValueIndex(MatchTypeName, 
			                                         class'GFxAlloyFrontEnd_JoinGame'.const.SERVERBROWSER_SERVERTYPE_LAN);		
		}
...
function string GenerateMutatorURLString()
{
	...
	local AlloyUIDataStore_MenuItems MenuDataStore;
	...
	if ( DSClient != None )
	{
		MenuDataStore = AlloyUIDataStore_MenuItems(DSClient.FindDataStore('AlloyMenuItems'));
	...
	if (OnlineSub != None)
	{
		...
		if (GameInterface != None)
		{
			...
			if(bWasSuccessful)
			{
				...
				// Append any mutators
				Mutators = class'GFxAlloyFrontEnd_Mutators'.static.GetEnabledMutators();
				if(Len(Mutators) > 0)
				{

...
Filename    :   GFxAlloyFrontEnd_JoinGame.uc
...
**********************************************************************/
class GFxAlloyFrontEnd_JoinGame extends GFxAlloyFrontEnd_Screen;
...
var transient AlloyUIDataStore_StringList		StringListDataStore;

/** Reference to the game search datastore. */
var transient AlloyDataStore_GameSearch      SearchAlloyDataStore;
...
var private transient GFxAlloyFrontEnd_PasswordDialog PasswordDialog;

/** Reference to the Join Server dialog's MovieClip. */
var private transient GFxAlloyFrontEnd_JoinDialog     JoinDialogMC; 
...
function OnViewLoaded()
{
	...
	if ( DSClient != None )
	{
		...
		StringListDataStore = AlloyUIDataStore_StringList(DSClient.FindDataStore('AlloyStringList'));
	} else {
		`log("DSClient is none...");
	}
...
final function UpdateFilterButton()
{
	...
	local AlloyGameSettingsCommon GameSettings;
	...
	SettingsDataStore = UIDataStore_OnlineGameSettings(class'UIRoot'.static.StaticResolveDataStore('AlloyGameSettings'));
	GameSettings = AlloyGameSettingsCommon(SettingsDataStore.GetCurrentGameSettings());

	FilterGameMode = GameSettings.GetPropertyAsStringByName('CustomGameMode');
	FilterGameMode = class'GFxAlloyFrontEnd_LaunchGame'.static.GetGameModeFriendlyString(FilterGameMode);
...
{    
	if (MenuManager != none)
	{				
		PasswordDialog = GFxAlloyFrontEnd_PasswordDialog(MenuManager.SpawnDialog('PasswordDialog'));    
...
{
	local GFxAlloyFrontEnd_FilterDialog FilterDialog;    
	FilterDialog = GFxAlloyFrontEnd_FilterDialog(MenuManager.SpawnDialog('FilterDialog', self));
...
function OnServerListItemPress(GFxClikWidget.EventData ev)
{
	...
	JoinDialogMC = GFxAlloyFrontEnd_JoinDialog(MenuManager.SpawnDialog('JoinDialog')); 
...
{
	local string URL;
	local GFxAlloyFrontEnd_ErrorDialog DialogMC;
	local AlloyGameSettingsCommon CurGameSettings;
	...
	if (GameInterface != None)
	{
		if (bSuccessful)
		{
			...
			if (GameInterface.GetResolvedConnectString('Game',URL))
			{
				...
				{
					CurGameSettings = AlloyGameSettingsCommon(OnlineGameInterfaceImpl(GameInterface).GameSettings);
				}
		...
		else
		{
			...
			if (MenuManager != none)
			{
				DialogMC = GFxAlloyFrontEnd_ErrorDialog(MenuManager.SpawnDialog('ErrorDialog'));
...
function OnCancelSearchComplete( bool bWasSuccessful )
{
	...
	else if ( QueryCompletionAction != QUERYACTION_None )
	{
		...
		if (MenuManager != none)
		{				
			Dialog = GFxAlloyFrontEnd_ErrorDialog(MenuManager.SpawnDialog('ErrorDialog'));
...
event bool WidgetInitialized(name WidgetName, name WidgetPath, GFxObject Widget)
{
	...
	// `log("GFxAlloyFrontEnd_JoinGame: WidgetInit - WidgetName: " @ WidgetName @ " : " @ WidgetPath @ " : " @ Widget);
	switch(WidgetName)
	{
		case ('list'): 
...
final function ForceLANOption()
{
	...
	local AlloyGameSearchCustom CustomSearch;
	local array<UDKUIResourceDataProvider> Providers;
	local AlloyUIDataProvider_GameModeInfo Provider;
	...
	// LAN queries show all gametypes, so switch to the 'AlloyGameSearchCustom' (i.e. 'All Gametypes') search mode
	Class'AlloyUIDataStore_MenuItems'.static.GetAllResourceDataProviders(Class'AlloyUIDataProvider_GameModeInfo', Providers);

	ValueIndex = INDEX_None;

	for (i=0; i<Providers.Length; ++i)
	{
		Provider = AlloyUIDataProvider_GameModeInfo(Providers[i]);

		if (Provider != none && Provider.GameSearchClass == "AlloyGameSearchCustom")
		{
			ValueIndex = i;
			break;
		}
	...
	{
		ValueIndex = SearchDataStore.FindSearchConfigurationIndex('AlloyGameSearchCustom');

		if (ValueIndex != INDEX_None)
		{
			CustomSearch = AlloyGameSearchCustom(SearchDataStore.GameSearchCfgList[ValueIndex].Search);
			CustomSearch.ResetFilters();

			SearchDataStore.SetCurrentByName('AlloyGameSearchCustom', true);
		}
	}
}
...
function OnGameTypeChanged( optional int PlayerIndex )
{
	...
	local UIDataStore_OnlineGameSettings SettingsDataStore;
	local AlloyGameSearchCustom CustomSearch;
	local AlloyGameSearchCommon CommonSearch;
	local array<UDKUIResourceDataProvider> Providers;
	local AlloyUIDataProvider_GameModeInfo Provider;
	...
	{
		CommonSearch = AlloyGameSearchCommon(SearchDataStore.GetCurrentGameSearch());
	...
	if (GameTypeClassName != "")
	{
		...
		{
			i = SearchDataStore.FindSearchConfigurationIndex('AlloyGameSearchCustom');

			if (i != INDEX_None)
			{
				CustomSearch = AlloyGameSearchCustom(SearchDataStore.GameSearchCfgList[i].Search);
				CustomSearch.ResetFilters();

				// The default state of 'AlloyGameSearchCustom' is to list all gametypes; only add gametype filters if needed
			...
			{
				`log("Error, AlloyGameSearchCustom is not set in GameSearchCfgList");
			}
		}
		else
		{
			...
			if (ProviderIdx != INDEX_None)
			{
				...
				CommonSearch = AlloyGameSearchCommon(SearchDataStore.GameSearchCfgList[ProviderIdx].Search);

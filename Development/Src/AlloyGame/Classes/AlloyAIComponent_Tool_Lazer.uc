class AlloyAIComponent_Tool_Lazer extends AlloyAIComponent_Tool;
...
//		tool_hammer_anim_attack
		...
		AlloyBotPawn(toolController.Pawn).ToolPart.PlayAnim('tool_hammer_lazer_attack');
...
//		`Log("Hammer Collecting");
		AlloyBotPawn(toolController.Pawn).ToolPart.PlayAnim('tool_lazer_anim_attack');
		Cooldown = 15.0f;
	} else {
		Cooldown = Cooldown - 1;
	}
...
//		`Log("Hammer Being Hit");
		AlloyBotPawn(toolController.Pawn).ToolPart.PlayAnim('tool_lazer_anim_hit');
		Cooldown = 15.0f;
	} else {
		Cooldown = Cooldown - 1;
	}
...
defaultproperties
{
	WalkingAnim = "tool_lazer_anim_idle"
	Cooldown = 15.0
	ToolRange = 500
	ToolDamage = 10

 ...
 */
class AlloyDataStore_GameSearch extends UDKDataStore_GameSearchBase
	config(UI);

var			class<AlloyDataStore_GameSearchHistory>	HistoryGameSearchDataStoreClass;
...
event Registered( LocalPlayer PlayerOwner )
{
	...
	if ( DSClient != None )
	{
		...
		{
			HistoryGameSearchDataStoreClass = class'AlloyGame.AlloyDataStore_GameSearchHistory';
		}
		...
		if ( Search != None )
		{
			...
			if ( StoredSearchIndex != INDEX_NONE )
			{
				...
				for ( SettingIndex = 0; SettingIndex < Search.LocalizedSettings.Length; SettingIndex++ )
				{
					...
					if ( SettingId != class'AlloyGameSearchCommon'.const.CONTEXT_GAME_MODE )
					{
...
DefaultProperties
{
	...
	GameSearchCfgList.Add((GameSearchClass=class'AlloyGame.AlloyGameSearch',DefaultGameSettingsClass=class'AlloyGame.AlloyGameSettings',SearchResultsProviderClass=class'AlloyGame.AlloyUIDataProvider_SearchResult',SearchName="AlloyGameSearch"))
}
